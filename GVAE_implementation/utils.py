...
def slice_edge_type_from_edge_feats(edge_feats):
    ...
    """
    print(edge_feats.shape, 'Edge Feature Initial')
    ...
    print(edge_types.shape, 'Edge Ty')
    return edge_types
...
def approximate_recon_loss(node_targets, node_preds, triu_targets, triu_preds):
    ...
    print(node_preds_matrix.shape, 'Node Pred Matrix')

    # Reshape triu predictions
    #FIXME:  len(SUPPORTED_EDGES) + 1 before but have removed it for momentary
    ...
    print(node_preds_reduced.shape, node_targets_reduced.shape, 'Node Preds Reduced and Target')
    ...
    approx_loss =   node_loss  + edge_loss
...
def gvae_loss(triu_logits, node_logits, edge_index, edge_types, node_types, \
    ...
    print(edge_index.shape, edge_index[0].shape, edge_index[1].shape, edge_types.shape, '==')
